...
public class Race
{
	...
	private static void initRaceData()
	{
		...
		packRaces[DefaultRace.Dwarf__Hill.ordinal()] = new Race(0, 0, 2, 0, 1, 0, 0, "Hill Dwarf", "Bold and hardy, dwarves are known as skilled warriors, miners, and workers of stone and metal. Though they stand well under 5 feet tall, dwarves are so broad and compact that they can weigh as much as a human standing nearly two feet taller. Their courage and endurance are also easily a match for any of the larger folk. As a hill dwarf, you have keen senses, deep intuition, and remarkable resilience.", "Can live to be over 400 years old. This longevity gives them a unique perspective compared to shorter-lived races.", "Skin color: Ranges from deep brown to paler hues tinged with red. Most common- light brown or deep tan. Hair: Worn long but simply styled. Usually black, gray, or brown - though paler dwarves often have red hair. Male dwarves highly value their beards.", "Dwarven kingdoms stretch deep beneath the mountains where the dwarves mine gems and precious metals and forge " +
	...
	public Race copy()
	{
		String[] traits_clone = (new ArrayList<>(traits).toArray(new String[traits.size()]));
		return new Race(str, dex, con, intel, wis, cha, assignable_points, name, description, description_age, description_physical, description_society, trait_age, trait_alignment, trait_size, trait_speed, traits_clone, languages);
	}
	
	public static Race getRace(int ind)
	{
		if(ind < 0) return null;
		if(ind < packRaces.length)
			return packRaces[ind];
		ind -= packRaces.length;
		if(ind > customRaces.size())
			return null;
		return customRaces.get(ind);
	}
	
	public static int getIndex(Race race)
	{
		int index = -1;
		if(race == null) return index;
		for(Race r : packRaces)
		{
			++index;
			if(r == race) return index;
		}
		for(Race r : customRaces)
		{
			++index;
	...
	{
		return packRaces.length + customRaces.size();
	}
	
	public void setAll(Race other)
	{
		str = other.str;
		dex = other.dex;
		con = other.con;
		...
		wis = other.wis;
		cha = other.cha;
		assignable_points = other.assignable_points;
		name = other.name;
		description = other.description;
		description_age = other.description_age;
		description_physical = other.description_physical;
		description_society = other.description_society;
		trait_age = other.trait_age;
		trait_alignment = other.trait_alignment;
		trait_size = other.trait_size;
		trait_speed = other.trait_speed;
		traits.clear(); traits.addAll(other.traits);
		languages = other.languages;
	}
	
	public static void setIndex(int index, Race src)
	{
		Race dest = getRace(index);
		if(dest == null)
		{
			customRaces.add(src);
		}
